        ...
        <ProtectedRoute allowedRoles={['CHURCH_ADMIN']}>
            <div className="space-y-6 max-w-6xl mx-auto pb-12">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center p-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 print:hidden">
                        ...
                        <p className="text-slate-400 text-sm mt-1">Results will appear here once explicitly released by system administrators.</p>
                    </div>
                ) : (
                    <div className="space-y-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden print:border-none print:shadow-none print:break-inside-avoid shadow-sm p-6 print:p-0">
                        {filteredExams.map(examTitle => {
                            ...
                            return (
                                <div key={examTitle} className={`space-y-6 print:break-inside-avoid ${!showExam ? 'print:hidden' : ''}`}>
                                    {/* Exam Title */}
                                    <h3 className="text-2xl font-black text-slate-800 dark:text-slate-100 flex items-center justify-between border-b-2 border-blue-500 pb-3">
                                        <div className="flex items-center gap-2">
                                            <FileText className="w-8 h-8 text-blue-600 dark:text-blue-400 print:hidden" />
                                            {examTitle}
                                        </div>
                                        <button onClick={() => handlePrint(examId)} className="print:hidden p-2 rounded-lg text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-700 dark:hover:text-slate-200 transition-colors" title={`Print ${examTitle} Report`}>
                                    ...
                                    {Object.keys(report[examTitle]).map(rankName => {
                                        ...
                                        return (
                                            <div key={rankName} className={`space-y-4 pt-4 ${!showRank ? 'print:hidden' : ''}`}>
            ...
            <style jsx global>{`
                @media print {
                    ...
                    /* Hide sidebar and top header during print */
                    nav, aside, header { display: none !important; }
                    /* Force main container to occupy full width and fix padding */
                    main, .flex-1 { padding: 0 !important; margin: 0 !important; width: 100% !important; max-width: 100% !important; }
