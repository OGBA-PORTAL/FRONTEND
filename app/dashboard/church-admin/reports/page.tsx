...
import { Loader2, Printer, Search, Award, FileText, Eye, X, CheckCircle, XCircle, Users, LayoutList } from 'lucide-react';
...
export default function ChurchAdminReportsPage() {
    ...
    const [activeTab, setActiveTab] = useState<'exam' | 'rank'>('exam');
            ...
            <div className="space-y-6 max-w-5xl mx-auto pb-12">
                ...
                <div className="no-print bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b border-slate-100 dark:border-slate-800">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Church Exam Reports</h1>
                            <p className="text-slate-500 text-sm mt-1">Hierarchical performance breakdown for your church.</p>
                        </div>
                        <div className="flex flex-wrap gap-2 w-full md:w-auto">
                            ...
                            <button onClick={() => handlePrint(null)}
                                ...
                                <Printer className="w-4 h-4" /> Print {activeTab === 'exam' ? 'All Exams' : 'All Ranks'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex px-4 pt-4 gap-2 overflow-x-auto bg-slate-50/50 dark:bg-slate-900/50">
                        <button onClick={() => { setActiveTab('exam'); setSearchTerm(''); }}
                            className={`flex items-center gap-2 px-5 py-3 border-b-2 font-medium text-sm transition-colors whitespace-nowrap ${activeTab === 'exam' ? 'border-blue-600 text-blue-700 dark:text-blue-400 bg-white dark:bg-slate-800/50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}>
                            <LayoutList className="w-4 h-4" /> Group by Exam
                        </button>
                                    ...
                                    <div key={examTitle}
                                        ...
                                        <div style={{ background: '#1e40af', color: 'white', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                ...
                                                <div>
                                                    <h2 style={{ fontWeight: 900, fontSize: 16, textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>{examTitle}</h2>
                                                    <p className="no-print" style={{ fontSize: 11, opacity: 0.75, margin: 0 }}>
                                                        {Object.keys(report[examTitle]).length} rank(s)
                                                    </p>
                                                </div>
                                            </div>
                                    ...
                                    <div key={rankName}
                                        ...
                                        <div style={{ background: '#0f172a', color: 'white', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                ...
                                                <div>
                                                    ...
                                                    <p className="no-print" style={{ fontSize: 11, opacity: 0.75, margin: 0 }}>
                                                        {Object.keys(groupedByRank[rankName]).length} exam(s)
                                                    </p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="p-5 space-y-10">
                                            {Object.keys(groupedByRank[rankName]).map(examTitle => {
                                                ...
                                                // Always show all exams under a printed rank
                                                const group = groupedByRank[rankName][examTitle];
                                                
                                                return (
                                                    <div key={examTitle}>
                                                        <div className="space-y-6">
                                                            <div className={`rpt-rank`}>
                                                                {/* Exam stats bar */}
                                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: '#f8fafc', border: '1px solid #e2e8f0', padding: '10px 14px', marginBottom: 6 }}>
